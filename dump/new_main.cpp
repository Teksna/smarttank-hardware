...
// #define SENSOR_MAX_PERCENT 78  // adjust based on your tank
...
//     if (!bufferFilled) return newValue;
...
//     // ❌ Reject spike
//     if ((maxVal - minVal) > MAX_VARIATION) {
//         Serial.println("⚠️ Spike detected → ignoring");
//         return readings[(readIndex + FILTER_SIZE - 2) % FILTER_SIZE];
//     }
...
//         // RAW %
//         int raw_level = (waterLevel / tankHeight) * 100;

//         if (raw_level > 100) raw_level = 100;
//         if (raw_level < 0) raw_level = 0;

//         // 🔥 CALIBRATION (fix sensor limit)
//         int calibrated_level = (raw_level * 100) / SENSOR_MAX_PERCENT;

//         if (calibrated_level > 100) calibrated_level = 100;

//         // 🔥 FILTER (remove noise)
//         int water_level = getFilteredLevel(calibrated_level);

//         // 🔥 STABILITY CHECK (prevent false triggers)
//         if (abs(water_level - lastStableLevel) < 3) {
//             stableCount++;
//         } else {
//             stableCount = 0;
//         }
...
//             return; // ⛔ skip rest of loop (no motor action)
//         }
